    ...
    for i in range(len(df)):
        if df.at[i, '姿態轉換(Y/N)'] == 'Y' and i not in processed_rows:
            if i > 0:
                ...
                prev_sattype = df.at[i - 1, '衛星姿態']
                curr_sattype = df.at[i, '衛星姿態']
                
                # 如果優先級變化，更新前後 10 行的 '衛星姿態' 和 '姿態轉換(Y/N)'
                if curr_priority < prev_priority:
                    # 往上更新，範圍往上移一格
                    start_idx = max(0, i - 11)  # 注意這裡的 -11，保證範圍向上一格
                    end_idx = i - 1  # 更新範圍往上一格
                    df.loc[start_idx:end_idx, '衛星姿態'] = curr_sattype
                    df.loc[start_idx:end_idx, '姿態轉換(Y/N)'] = 'Y'
                    df.loc[start_idx:end_idx, 'priority'] = curr_priority
                    # 將處理過的行添加到集合中
                    ...
                    highest_priority = max(df.loc[start_idx:end_idx, 'priority'])
                    highest_sattype = df.loc[df['priority'] == highest_priority, '衛星姿態'].iloc[0]
                    df.loc[start_idx - 1:end_idx - 1, '衛星姿態'] = prev_sattype  # 往下更新範圍整體向上一格
                    df.loc[start_idx - 1:end_idx - 1, '姿態轉換(Y/N)'] = 'Y'
                    df.loc[start_idx - 1:end_idx - 1, 'priority'] = prev_priority
                    # 將處理過的行添加到集合中
                    processed_rows.update(range(start_idx - 1, end_idx - 1))
